...
try {
    ...
    // Handle GET /deals/{id}/debug - Get deal debug information (includes agent_email_sent)
    elseif ($method === 'GET' && preg_match('/^\/deals\/(\d+)\/debug$/', $path, $matches)) {
        $deal_id = $matches[1];
        
        try {
            $pdo = Database::getConnection();
            
            // Get deal with all fields including agent_email_sent
            ...
            $response = [
                ...
                'can_confirm_rights' => ((bool)$deal['transaction_fee_paid'] && (bool)$deal['agent_email_sent']),
            ...
            error_log('Deal debug error: ' . $e->getMessage());
            http_response_code(500);
            echo json_encode(['success' => false, 'message' => 'Server error: ' . $e->getMessage()]);
            exit;
        }
