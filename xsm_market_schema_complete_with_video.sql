...
-- Updated with video support for chat messages
...
-- =============================================================================
-- TABLE: messages
-- Purpose: Store individual messages within chats (UPDATED WITH VIDEO SUPPORT)
-- =============================================================================
...
CREATE INDEX `idx_messages_chat_type_created` ON `messages` (`chatId`, `messageType`, `createdAt`);
CREATE INDEX `idx_messages_media_url` ON `messages` (`mediaUrl`);
CREATE INDEX `idx_users_auth_provider` ON `users` (`authProvider`);
CREATE INDEX `idx_ads_platform_status_price` ON `ads` (`platform`, `status`, `price`);

-- =============================================================================
-- VIDEO SUPPORT CONFIGURATION
-- =============================================================================

-- This schema includes full video support for the chat system:
-- 1. messageType enum includes 'video' option
-- 2. mediaUrl field stores video file paths
-- 3. fileName stores original video filename
-- 4. fileSize stores video file size (changed to bigint for large videos)
-- 5. thumbnail field can store video thumbnail paths
-- 6. Additional indexes for media-related queries
