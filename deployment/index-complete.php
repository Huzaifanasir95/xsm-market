...
require_once __DIR__ . '/controllers/UserController-complete.php';
...
try {
    ...
    elseif ($path === '/health' || $path === '/') {
        http_response_code(200);
        echo json_encode(['status' => 'ok', 'message' => 'PHP Backend API is running']);
    }
    else {
...
function handleAuthRoutes($controller, $path, $method) {
    switch ($path) {
        ...
        case '/auth/resend-otp':
            ...
            break;
        case '/auth/google':
            if ($method === 'POST') $controller->googleAuth();
            else methodNotAllowed();
            break;
...
function handleAdRoutes($controller, $path, $method) {
    if ($path === '/ads' || $path === '/ads/') {
        if ($method === 'GET') $controller->getAds();
